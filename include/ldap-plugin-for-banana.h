...
namespace ElasticClient
{
		...
		, const int maxBatchSize = 100000000)
	{
		...
		for (auto e = begin; e != end; ++e)
		{
			...
			if (1 + e == end || s.tellp() >= maxBatchSize)
			{
				HttpClient client(url);
				auto response = client.request("PUT", "/" + index + "/" + type + "/_bulk", s);
		...
		, const string &type)
	{
		...
		HttpClient client(url);
		auto response = client.request("DELETE", "/" + index + "/" + type, s);
		s.str(""); s << response->content.rdbuf();
		spdlog::get("logger")->info() << s.str();
	}
		...
		, const string &type)
	{
		...
		if (begin != end)
		{
			...
			HttpClient client(url);
			auto response = client.request("GET", "/" + index + "/" + type + "/_mget", s);
			s.str(""); s << response->content.rdbuf();
			spdlog::get("logger")->info() << s.str();
		}
		...
		, const string &type)
	{
		...
		stringstream s;
		HttpClient client(url);
		auto response = client.request("GET", "/" + index + "/" + type + "/_search", s);
		s.str(""); s << response->content.rdbuf();
		spdlog::get("logger")->info() << s.str();
		return Json::parse(s.str(), error);
	}
...
void ExecutePeriodicUpsert()
{
	...
	const string ldapCreateTimeAttribute = "";
	const string ldapUpdateTimeAttribute = "";
	//const time_t ldapLowerBound = LdapDateTime::ToTimeT("20150808200000.0Z"); // -1
	//const time_t ldapUpperBound = LdapDateTime::ToTimeT("20150809000000.0Z"); // -1
